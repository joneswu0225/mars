...
public class HuoshanController extends BaseController {
    ...
    static {
        File path = new File(ApplicationConst.UPLOAD_PATH + "/" + SWAPFACE_REL_PATH);
        if(!path.exists()){
            path.mkdirs();
        }
    }
    ...
    public BaseResponse swapface(
        ...
        try {
            ...
            String filePath = ApplicationConst.UPLOAD_PATH + "/" + SWAPFACE_REL_PATH + "/" + fileName;
    ...
    public BaseResponse swapfaceParam(@RequestBody @ApiParam(required=true) HuoshanSwapfaceParam param) throws Exception{
        ...
        if(!StringUtils.isEmpty(param.getTemplatePath())){
            param.setTemplateBase64(ObjectUtil.getBase64FromInputStream(Files.newInputStream(Paths.get(ApplicationConst.UPLOAD_PATH, param.getTemplatePath()))));
        }
