...
public class AliTTsUtil {
    private static final Logger logger = LoggerFactory.getLogger(AliTTsUtil.class);
    private static long startTime;
    private String appKey;
    NlsClient client;

    /// 直接传递token进来
    public AliTTsUtil(String appKey, String token) {
        this.appKey = appKey;
        //TODO 重要提示 创建NlsClient实例,应用全局创建一个即可,生命周期可和整个应用保持一致,默认服务地址为阿里云线上服务地址
        client = new NlsClient(token);
    }

    public AliTTsUtil(String appKey, String accessKeyId, String accessKeySecret, String url) {
        this.appKey = appKey;
        //TODO 重要提示 创建NlsClient实例,应用全局创建一个即可,生命周期可和整个应用保持一致,默认服务地址为阿里云线上服务地址
                ...
                public void onMessage(ByteBuffer message) {
                    try {
                        if(firstRecvBinary) {
                            ...
                            logger.info("tts first latency : " + (now - AliTTsUtil.startTime) + " ms");
                        }
    ...
    public void process() {
        ...
        try {
            ...
            AliTTsUtil.startTime = System.currentTimeMillis();
...
//            System.exit(-1);
        ...
        AliTTsUtil demo = new AliTTsUtil(appKey, id, secret, url);
        demo.process();
        demo.shutdown();
    }
}
